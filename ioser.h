    ...
    */
    ...
    inline void bufflush()
    {
        ...
        do{
            if(count > 0)
            {
                cout<<"should flush "<<rest<<" bytes, but flushes only "<<bytes_written<<" bytes !!!"<<endl;
                perror("Error printed by perror");
                assert(count == MAX_FWRITE_TRIALS);
                //------
                rest -= bytes_written;
                curpos += bytes_written;
                cout<<"try to write the remaining "<<rest<<" bytes ..."<<endl;
            }
...
ifbinstream & operator<<(ifbinstream & m, float i)
{
...
template <class KeyT, class ValT>
ifbinstream & operator<<(ifbinstream & m, const unordered_map<KeyT, ValT> & v)
{
    m << v.size();
...
template <class T>
ifbinstream & operator<<(ifbinstream & m, const unordered_set<T> & v)
{
    m << v.size();
...
public:
	...
	ofbinstream(const char* path)
	{
        ...
        //######
		//get file size
		filesize = -1; //todo:: filesize is unsigned int, should not take this value
		struct stat statbuff;
		if(stat(path, &statbuff) == 0) filesize = statbuff.st_size;
        else{
            cout << "ERROR: filesize = " << filesize <<  " File " << __FILE__ << ", Line = " << __LINE__ << endl;
        }
		//get first batch
		fill();
		totpos = 0;
...
{
    i = *(int*)m.raw_bytes(sizeof(short));
    return m;
}
...
ofbinstream & operator>>(ofbinstream & m, float & i)
{
    i = *(double*)m.raw_bytes(sizeof(float));
    return m;
}
...
template <class KeyT, class ValT>
ofbinstream & operator>>(ofbinstream & m, unordered_map<KeyT, ValT> & v)
{
    size_t size;
    m >> size;
    for (size_t i = 0; i < size; i++) {
...
template <class T>
ofbinstream & operator>>(ofbinstream & m, unordered_set<T> & v)
{
    size_t size;
    m >> size;
    for (size_t i = 0; i < size; i++) {
